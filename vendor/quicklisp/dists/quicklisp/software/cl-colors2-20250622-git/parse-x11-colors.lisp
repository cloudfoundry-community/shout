  ...
  (let ((color-scanner (cl-ppcre:create-scanner
    ...
    (with-open-file (source source-path
      ...
      (with-open-file (colordefs destination-path
                                 ...
                                 :if-exists :supersede
                                 :if-does-not-exist :create)
        (format colordefs ";;;; This file was generated automatically by parse-x11.lisp
;;;; Please do not edit directly, just run make if necessary (but should not be).~2%")
        (pprint '(in-package :cl-colors2) colordefs)
          ...
          (do ((line (read-line source nil nil) (read-line source nil nil)))
            ...
            (unless (cl-ppcre:scan-to-strings comment-scanner line)
              (multiple-value-bind (match registers)
                ...
                (if match
                    (let* ((color-const (cl-colors2::colorname->constant-name (aref registers 3)))
                           (colorname
                            (cl-ppcre:regex-replace-all "\\+"
                                                        (string-downcase (symbol-name color-const))
                                                        ""))
                           (color-struct `(define-rgb-color ,colorname
                                              ,(parse-channel (aref registers 0))
                                            ,(parse-channel (aref registers 1))
                                            ,(parse-channel (aref registers 2)))))
                      (pprint `(push (cons ,colorname ,color-struct)
                                     *x11-colors-list*)
                              colordefs)
                      (format colordefs "~%")
